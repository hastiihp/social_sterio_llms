...
Output: health_prompts_full.csv, one row per persona x topic x condition
(5,400 x 7 x 2 = 75,600 rows -- same total scale as the main study's
prompts.csv), with the full conversation stored as a JSON-encoded list of
{"role","content"} messages in the `messages_json` column (ready to pass
as-is to tokenizer.apply_chat_template).
"""
import json
import pandas as pd

PERSONAS_CSV = "../data/personas.csv"
TOPICS_CSV = "../data/topics.csv"
OUT_CSV = "health_prompts_full.csv"
