"""
Full run of the health-conversation prompt variant: 5 models (same 5 that
succeeded in the main study; Falcon remains excluded, see
FALCON_EXCLUSION.md), all 5,400 personas x 7 topics x 2 conditions =
75,600 rows per model.
...
Run on a GPU node with inference/ as the working directory:
    ...
    python full_health_inference.py deepseek
    python full_health_inference.py ministral
"""
...
def build_batch_inputs(tokenizer, model_id, message_lists):
    """message_lists: list of full conversation (list of {role,content} dicts),
    one per row in the batch -- NOT single strings. This is the only
    substantive change from full_inference.py's build_batch_inputs."""
    texts = []
...
def run_full(model_key):
    ...
    print(f"\n{'=' * 70}\nHEALTH-CONVERSATION FULL RUN: {model_id}\n{'=' * 70}", flush=True)
    ...
    print(f"Health-conversation run complete for {model_id}: {n_written} rows written, "
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or sys.argv[1] not in MODELS:
        print(f"Usage: python full_health_inference.py [{'|'.join(MODELS.keys())}]")
        sys.exit(1)
    run_full(sys.argv[1])
    print("\nHealth-conversation run for this model complete.")
